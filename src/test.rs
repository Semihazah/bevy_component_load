...
use crate::{AppRegisterLoadExt, BevyComponentLoadPlugin, IsLoaded, Loadable};
...
fn setup_app(app: &mut App) {
    ...
    app.add_plugin(
        iyes_progress::ProgressPlugin::new(LoadingState::Loading)
...
fn test_loading() -> anyhow::Result<()> {
    let mut app = App::new();
    setup_app(&mut app);
    app.register_loadable::<Foo>();
    let foo_entity = app.world.spawn(Foo::default()).insert(IsLoaded).id();
    ...
    let foo = app
        .world
        .get::<Foo>(foo_entity)
        .ok_or(anyhow!("Unable to find Foo on entity!"))?;
